  ...
  static async fromResponse(response: Response): Promise<ApiError> {
    ...
    switch (response.status) {
      ...
      case 403:
        message =
          "Access denied: " +
          (data?.detail || "You don't have permission to access this resource");
        break;
      case 404:
        message =
          "Resource not found: " +
          (data?.detail || "The requested resource could not be found");
        break;
      case 422:
...
// Public API client that doesn't redirect on 401
...
// Helper function for GET requests that return JSON (no auth redirect)
export async function getJsonPublic<T>(path: string): Promise<T> {
  const response = await publicApi(path);
  return response.json();
}
